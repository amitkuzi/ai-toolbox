...
class Store:
    ...
    def append(
        ...
        is_initial_add = kind.endswith(".added")
        if not is_initial_add and not reason:
            raise ValueError(
                f"reason is required for kind '{kind}' (only an initial *.added event may omit it)"
            )
        event = {
            "event_id": new_ulid(),
            "ts": ts or now_iso(),
